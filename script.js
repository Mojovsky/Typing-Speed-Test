...
const addClass = (el, name) => (el.className += ` ${name}`);
const removeClass = (el, name) =>
  (el.className = el.className.replace(name, ""));
const randomWord = () => words[Math.ceil(Math.random() * wordsCount) - 1];
...
const getWpm = () => {
  ...
  const lastTypedWordIndex =
    words.indexOf(document.querySelector(".word.current")) + 1;
  const typedWords = words.slice(0, lastTypedWordIndex);
  const correctWords = typedWords.filter((word) => {
    const letters = [...word.children];
    const isCorrect = (letter) =>
      letter.className.includes("incorrect") === false &&
      letter.className.includes("correct");
    return letters.every(isCorrect);
  });
